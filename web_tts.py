...
# Cấu hình giao diện Web chuẩn
st.set_page_config(page_title="AI TTS Pro - Leonardotu", layout="wide")
        ...
        with st.status("Đang xử lý âm thanh... vui lòng chờ...", expanded=True) as status:
            try:
                ...
                if not is_srt:
                    ...
                    # Dịch file SRT (Ghép khớp mốc thời gian)
                ...
                st.error(f"Lỗi hệ thống: {e}. Hãy đảm bảo bạn đã cài ffmpeg trong packages.txt")
